...
import { getMyTripRegistrations } from "../../services/tripService";

export default function RegisteredTripsPage() {
  ...
  const loadRegistrations = async () => {
    try {
      ...
      alert("Failed to load registered trips");
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="min-h-screen bg-gradient-to-b from-orange-50 via-white to-yellow-50 px-4 py-10">
      <section className="mx-auto max-w-6xl">
        ...
        <p className="mb-8 text-slate-500">
          View all trips and yatras you registered for.
        </p>

        {registrations.length === 0 ? (
          <div className="rounded-3xl bg-white p-10 text-center shadow-sm">
            <p className="font-bold text-slate-700">
              You have not registered for any trip yet.
            </p>
          </div>
        ) : (
                  ...
                  <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-4 text-sm text-slate-500">
                      <span className="flex items-center gap-1">
                        ...
                        {reg.trip?.destination}
                      </span>

                      <span className="flex items-center gap-1">
                        <CalendarDays className="h-4 w-4" />
                        {formatDate(reg.trip?.start_date)} -{" "}
                        {formatDate(reg.trip?.end_date)}
                      </span>
