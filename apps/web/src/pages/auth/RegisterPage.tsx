        ...
        <div className="space-y-7 p-8">
          <Section title="Personal Information">
            <div className="grid gap-4 md:grid-cols-2">
              <Input label="First Name *" value={form.first_name} onChange={(v:any) => update("first_name", v)} />
              <Input label="Last Name" value={form.last_name} onChange={(v:any) => update("last_name", v)} />
              ...
              <Input label="Password *" type="password" value={form.password} onChange={(v:any) => update("password", v)} />

              <Select label="Gender" value={form.gender} onChange={(v:any) => update("gender", v)}>
          ...
          <Section title="Centre & Location">
            <div className="grid gap-4 md:grid-cols-2">
              <Select label="ISKCON Centre" value={String(form.centre_id)} onChange={(v:any) => update("centre_id", v)}>
              ...
              <Select label="Country" value={form.country_code} onChange={(v:any) => update("country_code", v)}>
              ...
              <Select label="State" value={form.state_code} onChange={(v:any) => update("state_code", v)}>
              ...
              <Select label="City" value={form.city} onChange={(v:any) => update("city", v)}>
          ...
          <Section title="Address Details">
            <div className="grid gap-4 md:grid-cols-2">
              <Input label="Address Line 1" value={form.address_line_1} onChange={(v:any) => update("address_line_1", v)} />
              <Input label="Address Line 2" value={form.address_line_2} onChange={(v:any) => update("address_line_2", v)} />
              <Input label="Landmark" value={form.landmark} onChange={(v:any) => update("landmark", v)} />
              <Input label="Postal Code" value={form.postal_code} onChange={(v:any) => update("postal_code", v)} />
              <div className="md:col-span-2">
                <Input label="Profile Image URL" value={form.profile_image_url} onChange={(v:any) => update("profile_image_url", v)} />
              </div>
            </div>
          </Section>
          ...
          {isDevotee && (
            <Section title="Devotee Request Details">
              <div className="grid gap-4 md:grid-cols-2">
                <Input label="Spiritual Name" value={form.spiritual_name} onChange={(v:any) => update("spiritual_name", v)} />
                <Input label="Current Malas" type="number" value={String(form.current_malas)} onChange={(v:any) => update("current_malas", Number(v))} />

                <Select label="Initiation Status" value={form.initiation_status} onChange={(v:any) => update("initiation_status", v)}>
                  <option value="none">None</option>
                  <option value="harinam">Harinam</option>
                  <option value="diksha">Diksha</option>
                </Select>

                <Input label="Years Associated" type="number" value={String(form.years_associated)} onChange={(v:any) => update("years_associated", Number(v))} />
                <Input label="Devotee Reference Name" value={form.devotee_reference_name} onChange={(v:any) => update("devotee_reference_name", v)} />
                <Input label="Devotee Reference Phone" value={form.devotee_reference_phone} onChange={(v:any) => update("devotee_reference_phone", v)} />

                <Textarea label="Services" value={form.services} onChange={(v:any) => update("services", v)} />
                <Textarea label="Reason for Devotee Request" value={form.reason} onChange={(v:any) => update("reason", v)} />
              </div>
            </Section>
          )}
