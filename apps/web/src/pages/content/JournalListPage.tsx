...
export default function JournalListPage() {
  ...
  const load = async () => {
    const res = await getPosts({ type: "journal" });
    setPosts(res.data || []);
  };
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <div className="mb-8 overflow-hidden rounded-[2rem] bg-gradient-to-br from-[#1a0a00] to-[#3d2200] p-8 text-white shadow-xl">
        ...
        <h1 className="mt-3 text-4xl font-black">Journal Library</h1>
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
        {filteredPosts.map((post) => (
          <JournalCard key={post.uuid} post={post} />
        ))}
      </div>
...
function JournalCard({ post }: { post: any }) {
